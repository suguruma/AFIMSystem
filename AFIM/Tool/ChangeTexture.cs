...
namespace AFIM
{
    public partial class ChangeTexture : Form
    {
        ...
        int read_max_point = 114;   // 読み込みの最大点数
        ...
        private void textrueImg_Click(object sender, EventArgs e)
        {
            openFileDialog3.FileName = CurImageName;
            openFileDialog3.Filter = ImageFilter;
            if (openFileDialog3.ShowDialog() == DialogResult.OK)
            {
                if (tmp_img != null)
                    tmp_img.Dispose();
                target_image = openFileDialog3.FileName;
                tmp_img = new Bitmap(Image.FromFile(target_image));
                pictureBox4.Image = Image.FromFile(target_image);
            }
        }

        private void textureFP_Click(object sender, EventArgs e)
        {
            openFileDialog4.FileName = CurFileName;
            openFileDialog4.Filter = FileFilter;
            if (openFileDialog4.ShowDialog() == DialogResult.OK)
            {
                target_pts = openFileDialog4.FileName;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDialog1.ShowDialog(this) == DialogResult.OK)
            {
                ...
                Directory.CreateDirectory(directory_path + save_filename_str);
                //Directory.CreateDirectory(directory_path + read_filename_str);
                for (int i = 0; i < 11; i++)
                {
                    frame_img[i].Save(directory_path + save_filename_str + "\\" + save_filename_str 
                        + "_" + i.ToString("00") + "." + save_extension_str);
        ...
        private void Transformation_Shape()
        {
            ...
            string strText = "";
            using (StreamReader sr = new StreamReader(@target_pts))
            {
                strText = sr.ReadToEnd();
            }
